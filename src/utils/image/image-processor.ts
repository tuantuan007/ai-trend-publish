...
export class WeixinImageProcessor {
     ...
     * @returns 压缩后的Buffer
     */
    private async compressImage(imageBuffer: ArrayBuffer, maxSizeInMB: number = 1): Promise<Uint8Array> {
        try {
            // 解码图片
    ...
    private extractImageUrls(content: string): string[] {
        ...
        const patterns = {
            markdown: /!\[[^\]]*\]\(([^)]+)\)/g,
            html: /<img[^>]+src=["']([^"']+)["'][^>]*>/g,
            plainUrl: /(https?:\/\/[^\s<>"]+?\/[^\s<>"]+?\.(jpg|jpeg|png|gif|webp))/gi
        };
    ...
    private replaceImageUrl(content: string, oldUrl: string, newUrl: string): string {
        ...
        return content
            .replace(new RegExp(`!\\[([^\\]]*)\\]\\(${escapedOldUrl}\\)`, 'g'), `![$1](${newUrl})`)
            .replace(new RegExp(`<img([^>]*)src=["']${escapedOldUrl}["']([^>]*)>`, 'g'), `<img$1src="${newUrl}"$2>`)
